...
- Calling a skill directly (`/tdd`, `/delegate`, …) skips Frame and Ask, but not Explore,
  Plan or Gate: explore enough that the plan names real files, then wait for "go".
...
| 4 | **Plan** | A short plan readable in under a minute that covers every write the task will make: numbered steps (each with how it gets checked), expected complexities and how to handle them, and a one-line routing. If the agent has a todo or plan tool, copy the plan into it (not a repo file — that copy isn't a repo write and needs no approval); otherwise the plan lives in the conversation. | — |
